        ...
        <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg">
            ...
            <div style="position:relative;top:250px;text-align:-webkit-center;overflow:unset;z-index:100">
                ...
                <div class="row mt-3">
                    <div class="col-12">
                        <div class="card">
                            <h5 class="card-header bg-white mb-0">Total Team List</h5>
                                ...
                                <div class="dataTable-search"><input class="dataTable-input" placeholder="Search..."
                                    ...
                                    <!--<div class="dataTable-container">
                                        <table class="table table-flush dataTable-table" id="datatable-search">
                                            ...
                                            <tbody v-if="downlines.length > 0">
                                                <tr v-for="downline in downlines">
                                                <td>{{downline.id}}</td>
                                                <td>{{downline.level}}</td>
                                                <td>{{downline.user.name}}</td>
                                                <td>{{downline.user_id}}</td>
                                                <td>{{downline.package.title}}</td>
                                                <td>{{new Date(downline.user.paid_date).toLocaleString()}}</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>-->
...
export default {
    ...
    computed: {
       itemsWithSno() {
           return this.downlines.map((d, index) => ({ ...d, sno: index + 1 }))
       }
   },
